...
operate all nplus instances in this namespace. There must be exactly one deployed
instance of this environment chart. Without the environment, the instance and component
charts will fail to deploy.
...
```console
$ helm install my-release nplus/nplus-environment
```
...
|-----|-------------|---------|
global.environment.ingress | defines environment-wide ingress values | `{}`, see section "Ingress" |
global.environment.storage.data.class | defines environment-wide data storage class see section "Storage" | unset, so using Kubernetes default class |
global.environment.storage.file.class | defines environment-wide file storage class see section "Storage" | unset, so using Kubernetes default class |
