...
The nscale Storage Layer provides an abstracted compliant blob storage api for
the nscale Application Layer. You can connect multiple storage subsystems like S3,
Azure Blob Storage and also Hardware Stores like NetApp, EMC Centera etc.
...
```console
$ helm install my-release nplus/nplus-component-nstl
```
...
|-----|-------------|---------|
checkHighestDocId | enables checking the highest DocID when starting the server. this only makes sense, if you also set a separate volume for the highest ID This is a backup / restore feature to avoid data mangling | `"0"` |
dvCheckPath | sets the path of the highest ID file. |  |
image.name |  | `"storage-layer"` |
image.repo |  | `"ceyoniq.azurecr.io/release/nscale"` |
image.tag |  | `"ubi.9.1.1200.2023112112"` |
